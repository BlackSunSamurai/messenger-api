...
class ConversationController(val conversationServiceImp: ConversationServiceImp,
    ...
    fun list(request: HttpServletRequest): ResponseEntity<ConversationListVO>{
        ...
        val conversations = conversationServiceImp.listUserConversation(user.id)
    ...
    request: HttpServletRequest): ResponseEntity<ConversationVO>{
        val user = userRepository.findByUsername(request.userPrincipal.name) as User
        val conversationThread = conversationServiceImp.retrieveThread(conversationId)
